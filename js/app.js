$(document).ready(function() {
    ...
    $('#nextquestion').click(function() {
        ...
        if (wine.questions[currentQuestion].correctAnswer == userInput) {
            score++;
        }
        currentQuestion++;
        if (currentQuestion == wine.questions.length) {
    ...
    function updateView() {
        $('#questionMain').html(wine.questions[currentQuestion].main);
        ...
        for (i = 0; i < 4; i++) {
            var radio = $(radios[i]);
            radio.text(wine.questions[currentQuestion].answers[i]);
        }
